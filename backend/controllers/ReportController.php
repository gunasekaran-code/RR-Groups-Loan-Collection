...
class ReportController extends Controller
{
    ...
    private function netBalance(): void
    {
        ...
        // 3. Chit Members (paid or partial)
        ...
        $stmt->execute();
        $activeLoansRow = $stmt->fetch();
        $activeLoansCount = (int)$activeLoansRow['count'];
        $activeLoanOutstanding = (float)$activeLoansRow['outstanding'];

        // 7. Overdue Loans
        $stmt = $pdo->prepare("SELECT COUNT(*) as count, COALESCE(SUM(outstanding_balance), 0) as outstanding FROM loans WHERE status = 'overdue'");
        $stmt->execute();
        $overdueLoansRow = $stmt->fetch();
        $overdueLoansCount = (int)$overdueLoansRow['count'];
        $overdueLoanOutstanding = (float)$overdueLoansRow['outstanding'];
        ...
        $stmt->execute();
        $chitRow = $stmt->fetch();
        $activeChitsCount = (int)$chitRow['count'];
        $chitGroupValue = (float)$chitRow['group_val'];
        $chitCollectedAmount = (float)$chitRow['coll'];
        $chitPendingAmount = (float)$chitRow['pend'];
        ...
        $stmt->execute();
        $fundRow = $stmt->fetch();
        $activeFundsCount = (int)$fundRow['count'];
        $fundTotalDeposit = (float)$fundRow['total_dep'];
        $fundCollectedAmount = (float)$fundRow['coll'];
        $fundPendingAmount = max(0, $fundTotalDeposit - $fundCollectedAmount);

        // 10. Ledger Custom Lent
        ...
        $outstandingMoneyLent = $totalLoanOutstanding + $chitPendingAmount + $fundPendingAmount + max(0, $customLentOut - $customLentCollected);
